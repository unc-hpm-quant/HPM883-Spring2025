...
- **No formula needed**
- Can capture complex relationships without parametric assumptions
...
2. For each combination, split predictor space into two regions:
   $$R_1(j,s) = \{X|X_j < s\} \text{ and } R_2(j,s) = \{X|X_j \geq s\}$$
3. Choose split that minimizes:
...
**By averaging over many low bias but high variance
models, we reduce the overall variance ⇒ more
accurate model.**
...
- the majority vote by all $B$ predictions for classification trees:
  $\hat{y}_{bag}(X) = \underset{k}{\arg\max} \sum_{i=1}^{B} I\{\hat{y}^i(X) = k\}$
...
## Random Forests: Decorrelating Trees (De-cor-relating, not de-cor-ating!)
