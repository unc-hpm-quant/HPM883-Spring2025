...
Now, _prior to treatment_ there are two **Potential Outcomes** for an individual:
...
Note that potential outcomes are different from realized outcomes. The potential outcomes are defined even _prior to treatment_. One an individual is assigned to treatment or control wee can then observe their realized outcome.
...
This could be due to a choice. Did a doctor choose to do it? Did the patient choose to do it? Did an
algorithm choose to do it? 
...
To see this more concretely, consider the following example: Assume a doctor sees patients, knows each
person’s treatment effects, despite counterfactuals, and assigns treatment based on whether gains are positive or not.^[This example is adapted from Scott Cunninham's example in the Mixtape book.]
